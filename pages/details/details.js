...
Page({
        ...
        if(!!samegoods[item].clickflavor){
          ...
          for(var i=0;i<newflavor.length;i++){
            if(this.data.flavor[i].flavor==samegoods[item].clickflavor){
              ...
              newflavor[i].flavornum = samegoods[item].count
              
              let newcount = samegoods[item].count
              this.setData({
                flavor:newflavor,
                buysum:this.data.buysum+newcount
              })
            }
          ...
          this.setData({
            goodscount:newcount,
            buysum:Number(samegoods[0].count)
          })
        }
        this.calculateprice()
      }
  ...
  tocal(){
    // 标记
    console.log('是否存在口味',!!this.data.flavor.length)
      ...
      for(var i=0;i<goods.length;i++){
        ...
        // 相同商品但口味不一样
          ...
          orderlist = {
            ...
            opriceGood:Number(this.data.rule[0].price).toFixed(2),
            stock:stock,
      ...
      if(!!theMsg.length){
        for(var j=0;j<API.orderinfo.length;j++){
          if(API.orderinfo[j]==theMsg[0]){
            let newsingleprice = 0
            //更改为实时数量
            let newcount = Number(this.data.buysum)
  ...
  getDate:function() {
    ...
    app.globalData.db.collection('goods').where({
      ...
      //好标
      console.log('orderinfo',API.orderinfo)
      this.setData({
        goodmsg: res.data[0],
  ...
  bindinput(e){
    if(!e.detail.value){
      ...
      if(this.data.goodscount-Number(e.detail.value)>=0){
        let sum = e.detail.value.replace(/\b(0+)/gi,"")
        this.setData({
          buysum: sum
        })
      }else{
